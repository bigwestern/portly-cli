...
def update_item_request(authenticated_portal, item_id, them_changes):
    ...
    # mixin
    post_params = {**them_changes, **update_item_params}
    ...
    url_parts = [portal_url.rstrip('/'), 'sharing/rest/content/users', owner, 'items', item_id, 'update']
    update_item_url = "/".join(url_parts)

    print("Posting content to url: '%s'" % (update_item_url))
    
    req = Request(update_item_url, data=parameters)
    response = urlopen(req).read()
    return response

def update_item(authenticated_portal, item_id, them_changes):
    ...
    try:
        ...
        if 'success' in result:
            ...
            print('error in update item response for item %s' % (item_id))
            print('error message: "%s"' % (result['error']['message']))
            for detail in result['error']['details']:
                print(detail)
        else:
            print('error updating %s' % (item_id))
    except Exception as e:
        print('Exception posting item %s' % (item_id))
        print(e.message)
    return portal_id
